...
this script can generate an chroma_distance table for all violin/piano pairs
...
def chromadistance_worker(argslist, topdb = 30):
    ...
    try:
        # try to calculate chroma distance of derived stems, if no overlapped (len(merged_ab) ==0), error would occur
        chroma_a = librosa.feature.chroma_stft(y = merged_a.squeeze(), sr = rate)
        chroma_b = librosa.feature.chroma_stft(y = merged_b.squeeze(), sr = rate)
        a_chmean = np.mean(chroma_a, axis = 1)
        b_chmean = np.mean(chroma_b, axis = 1)
        print("===calculating chroma distance of {}, and {}===".format(Path(path_a).stem, Path(path_b).stem))
        eud = distance.euclidean(a_chmean, b_chmean)

        
    except Exception:
        print("Error in chromadistance ")
        return "Error corr"
    
    return eud, path_a, path_b
...
def main():
    ...
    pool = Pool(30)
    results = pool.map(chromadistance_worker, arglist)
    pool.close()
    pool.join()
